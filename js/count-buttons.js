      ...
      for(let i = 0; i < allThumbnails.length; i++) {
        if(evt.target.closest(".goods-stock__item").id === allThumbnails[i].dataset.id) { //Если id в карточке товара равно id в миниатюре товара, то
          ...
          if (count < 2) { // Если количество товара <2, то скрываем элемент с количеством товара
            ...
            countThumbnail.style.display = "block";
            countThumbnail.textContent = count;
          }
          if (evt.target.closest(".goods-stock__item").id == 2) { //т.к. в данном случае товар будет доставляться в разные дни
            countThumbnail.textContent = (count - 16); //16 - из макета: кол-во товаров, которое доставят в другой день
          }
        }
      }
  }
